...
Классические Vim или Emacs так же могут быть использованы.

## А зачем всё это? - вместо преамбулы

Данный проект - просто демонстрационный пример численного решения системы
квадрантных уравнений. Подобная задача (только в несколько более
общем виде) решается в каждом GPS/Глонасс приёмнике. Данную постановку
задачи можно охарактеризовать как "GPS наоборот".

Практически, подобная задача может потребоваться для:

- мониторинга перемещения людей, грузов, транспорта на заводских территориях
(ангарах, складах, цехах, портах) снабженных активными радиочастотными
метками (RFID);

- решения поисковой задачи, то есть определения координат радиомаяков
по сигналу нескольких радиоприёмников, установленных на летательных
аппаратах, например.

## TODO: Что ещё?

Для решения поисковой задачи, как правило, одной координатой можно пренебречь,
так как высота нахождения передатчика определяется рельефом местности.
Необходимо определить лишь координаты передатчика на поверхности
(плоские координаты). Иногда достаточно определить направление для поиска.
При этом приёмники могут не находится в одной плоскости с передатчиком
(задача не сводится просто к случаю с d=2). Допущение о том, что одна
из искомых пространственных координат передатчика (например Z) не имеет значения
может обеспечить снижение требований к числу приёмников. Если для измерения
декартовых координат одного "маячка" требуется четыре приёмника, можно
предположить, что для оценки только двух координат хватит и трех.
Аналогично работает GPS (для оценки координат на поверхности планеты,
без оценки высоты достаточно приёма сигнала от 3-х спутников).
Вероятно стоит проработать вопрос реализации алгоритма оценки возможных
координат передатчика по сигналам только от 2-3 приёмников.
