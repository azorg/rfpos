...
static void rfpos_f(
    ...
    const la_vector_t *x, // выбранная точка в пространстве и времени
  ...
  double *r)  // радиус вектор приёмника, если решение найдено [d]
{
  ...
  // найти минимальное значение t[i]
  tmin = t[0];
  for (i = 1; i < n; i++)
  ...
  for (i = 0;; i++)
  {
    ...
    // отладочная печать модуля f и dx FIXME
    dx = 0.;  
    for (j = 0; j <= d; j++)
    {
      q = self->dx.d[j];
